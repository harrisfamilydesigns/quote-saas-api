class Api::V1::MaterialRequestsController < Api::V1::BaseController
  ...
  def index
    ...
    render json: material_requests.map { |mr| MaterialRequestSerializer.new(mr).serialize }
  end
  
  # GET /api/v1/material_requests/:id
  def show
    # Create serialized data
    serialized_data = MaterialRequestSerializer.new(@material_request).serialize
    
    # Add related data as separate keys in the response
    response_data = {
      material_request: serialized_data,
      quotes: @material_request.quotes.map { |q| QuoteSerializer.new(q).serialize },
      invited_suppliers: @material_request.invited_suppliers.map { |s| SupplierSerializer.new(s).serialize }
    }
